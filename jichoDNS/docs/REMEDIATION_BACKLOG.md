...
| 9 | **Real dark-web / breach ingestion** | Dark-web + credential data are synthetic. The **credential** ingestor's mock is now gated behind an explicit `ALLOW_MOCK_DATA` flag (off in prod). The legacy `services/darkweb.py` `_generate_demo_*` service is **dead code** — it is never seeded into the live path (not "gated", just unused) and should be deleted as part of the ASM/dark-web consolidation (#6). Wiring real sources (HIBP/Intelligence X/Tor collectors) is a feature. | medium | 1–2 wks |

...
$CC "${COMMON[@]}" "$ROOT/src/cascade.cpp" \
    -lws2_32 -lntdll -ldbghelp -lpsapi -ladvapi32 -lcrypt32 \
    -o "$OUT/cascade.exe"
...
echo "[+] building mock_driver_test (native Linux/CI validator)"
g++ -O2 -std=c++17 "$ROOT/src/mock_driver_test.cpp" -o "$OUT/mock_driver_test" 2>/dev/null \
    && echo "    $OUT/mock_driver_test" \
    || echo "[!] native g++ not found; skipping mock_driver_test (Windows-only via MSVC)"
